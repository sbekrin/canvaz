...
export function updateDndStateOnOver(
  ...
  const hasWhiteList: boolean =
    spectro.accepts && Array.isArray(spectro.accepts);
  const hasChildNodeKey: boolean = lastDragOverNodeKey !== null;
  const targetIsAllowedHere: boolean =
    hasWhiteList &&
    spectro.accepts.some(
      type => type._spectroEnhancer === targetInstance.constructor
    );
 ...
 */
export function updateDndStateOnDrop(
  instance: Object,
  event: MouseEvent
): EditorDNDState {
  ...
  const isAllowedHere: boolean = newDndState.acceptableComponents.some(
    type => type._spectroEnhancer === targetInstance.constructor
  );
